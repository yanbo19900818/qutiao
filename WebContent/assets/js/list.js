...
function stateDist(id){
	var adGroupId='';
	var deleReturnPage=$("#deleReturnPage").val();

	var pathUrl=$("#pathUrl").val();
	var arcCheck=$("input[type='checkbox']").is(':checked');
	if(arcCheck==false) {
        alert("请选择您要操作的广告组！！！");
    }else{
    	 $('input[type="checkbox"][name="arcID"]:checked').each(
                 function() {
                	 adGroupId+=$(this).val()+",";   
                 }
             );      
    ...
    /*	for(i=0;i<document.form2.arcID.length;i++)
    	{
    		if(document.form2.arcID[i].checked)
    		{
    			adGroupId=adGroupId + arcID[i].value+",";
    			
    		}
    	}*/
    	location=""+pathUrl+"?adGroupId="+adGroupId+"&dist="+id+"&deleReturnPage="+deleReturnPage+"";
    }
